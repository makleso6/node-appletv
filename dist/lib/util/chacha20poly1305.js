...
exports.Chacha20Ctx = Chacha20Ctx;
function load32(x, i) {
...
exports.Poly1305Ctx = Poly1305Ctx;
...
function aead_mac(key, ciphertext, data) {
    ...
    const m = new Array(clen + dlen + 16);
    let i = dlen;
    for (; i--;)
        m[i] = data[i];
    store64(m, dlen, dlen);
...
function aead_decrypt(ctx, nonce, ciphertext, ad) {
    ...
    if (poly1305_verify(digest, mac) !== 1)
        return false;
    const out = [];
