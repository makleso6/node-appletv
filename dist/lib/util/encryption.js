...
function computePoly1305(cipherText, AAD, nonce, key) {
    ...
    const polyKey = sodium_native_1.api.crypto_stream_chacha20(32, nonce, key);
    const computed_hmac = sodium_native_1.api.crypto_onetimeauth(msg, polyKey);
    polyKey.fill(0);
...
function verifyAndDecrypt(cipherText, mac, AAD, nonce, key) {
    const matches = sodium_native_1.api.crypto_verify_16(mac, computePoly1305(cipherText, AAD, nonce, key));
    if (matches === 0) {
        return sodium_native_1.api
...
function encryptAndSeal(plainText, AAD, nonce, key) {
    const cipherText = sodium_native_1.api
